...
<div class="min-h-screen bg-nord0 p-4 sm:p-8">
	...
	>
		...
		<a
			href={resolve('/sessions/create')}
			class="bg-nord10 hover:bg-nord9 text-nord6 font-bold py-2 px-4 rounded transition text-center"
		>
			New Session
		</a>
	</header>
		...
		<table class="min-w-full divide-y divide-nord2">
			...
			<tbody class="bg-nord1 divide-y divide-nord2">
				{#each data.sessions as session (session.id)}
					<tr
						class="hover:bg-nord2 transition cursor-pointer"
					...
					>
						<td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-nord6">
							<a
								...
								class="text-nord8 hover:text-nord7 hover:underline"
							>
				...
				{:else}
					<tr
						><td colspan="5" class="px-6 py-10 text-center text-nord4 italic">No sessions found.</td
						></tr
					>
				{/each}
			</tbody>
